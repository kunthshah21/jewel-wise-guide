...
import { generateEarringReport } from "@/lib/reportGenerator";
import {
  earringInventory,
  getEarringsByLifecycleStage,
  getTotalStockValue,
  ...
  type EarringItem,
} from "@/data/earringData";

export default function EarringDeepDive() {
  ...
  const uniqueMetals = useMemo(() => {
    ...
    earringInventory.forEach((item) => metals.add(item.metal));
    return Array.from(metals).sort();
  }, []);

  const uniqueLocations = useMemo(() => {
    const locations = new Set<string>();
    earringInventory.forEach((item) => locations.add(item.stockLocation));
  ...
  const deadStockPercentage = useMemo(() => 
    ((deadStockItems.length / earringInventory.length) * 100).toFixed(1),
    [deadStockItems]
  );
  const fastMovingPercentage = useMemo(() =>
    ((fastMovingItems.length / earringInventory.length) * 100).toFixed(1),
    [fastMovingItems]
  );
  ...
  const typePerformance = useMemo(() => 
    distributionByType.map((type) => {
      const items = earringInventory.filter((item) => item.type === type.type);
  ...
  const designStylePerformance = useMemo(() => 
    distributionByDesignStyle.map((style) => {
      const items = earringInventory.filter((item) => item.designStyle === style.style);
      const avgVelocity = items.length > 0 
        ? items.reduce((sum, item) => sum + item.salesVelocity, 0) / items.length
        : 0;
  ...
  if (!earringInventory || earringInventory.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h2 className="text-2xl font-bold">No Data Available</h2>
          <p className="text-muted-foreground">Earring inventory data could not be loaded.</p>
  ...
  const renderInventoryTable = (items: EarringItem[]) => (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Earring Inventory Deep Dive</h1>
            <p className="text-muted-foreground mt-1">
              Comprehensive analysis of earring stock movement and performance
            </p>
          </div>
        </div>
        <Button
          onClick={async () => {
            try {
              await generateEarringReport({
                totalStockValue,
                totalItems: earringInventory.length,
                deadStockItems,
                deadStockValue,
                deadStockPercentage,
                fastMovingItems,
      ...
      <div className="relative overflow-hidden rounded-2xl bg-card p-8 md:p-10 shadow-lg border-2 border-primary/30">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-4">Earring Inventory Health Report</h2>

          <div className="space-y-2">
            <div className="bg-muted/50 rounded-lg p-4 border-l-4 border-primary">
              <p className="text-lg font-semibold text-foreground mb-2">📊 Overall Performance</p>
              <p className="text-foreground/90 leading-relaxed">
                Your earring inventory consists of {earringInventory.length} items valued at{" "}
            ...
            <div className="bg-muted/50 rounded-lg p-4 border-l-4 border-primary">
              ...
              <p className="text-foreground/90 leading-relaxed">
                {topPerforming.type} earrings are your star performers with {topPerforming.count} items generating
                strong sales. {worstPerforming.type} earrings need strategic intervention with only{" "}
            ...
            <div className="bg-muted/50 rounded-lg p-4 border-l-4 border-primary">
              ...
              <ul className="text-foreground/90 leading-relaxed space-y-1 list-disc list-inside">
                <li>Restock {topPerforming.type} earrings with {topPerformingDesign.style} designs - high demand detected</li>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <KPICard
          title="Total Earring Stock Value"
          value={formatIndianCurrency(totalStockValue)}
          change={`${earringInventory.length} items in inventory`}
          changeType="neutral"
          icon={Package}
        ...
        <KPICard
          ...
          change="Across all earring items"
          changeType="neutral"
          icon={TrendingDown}
        ...
        <CardContent>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="fast-moving" className="mt-6">
              ...
              <div className="mb-6 rounded-lg bg-gradient-to-br from-success/10 to-success/5 border-2 border-success/30 p-5">
                ...
                <div className="bg-card/50 rounded-lg p-4 border border-success/20">
                  <p className="text-foreground/90 leading-relaxed mb-3">
                    Your {fastMovingStock.length} fast-moving earrings (valued at {formatIndianCurrency(fastMovingValue)}) are selling at 
                    {avgSalesVelocity} units/month, indicating strong market demand. {fastMovingTopType} earrings are your top performers. 
              ...
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-foreground mb-2">Fast Moving Earrings</h3>
            ...
            <TabsContent value="slow-moving" className="mt-6">
              ...
              <div className="mb-6 rounded-lg bg-gradient-to-br from-warning/10 to-warning/5 border-2 border-warning/30 p-5">
                ...
                <div className="bg-card/50 rounded-lg p-4 border border-warning/20">
                  <p className="text-foreground/90 leading-relaxed mb-3">
                    Your {slowMovingStock.length} slow-moving earrings (valued at {formatIndianCurrency(slowMovingValue)}) have been in 
              ...
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-foreground mb-2">Slow Moving Earrings</h3>
                <p className="text-sm text-muted-foreground">
                ...
                <div className="bg-card/50 rounded-lg p-4 border border-destructive/20">
                  ...
                  <div className="mt-3 p-3 rounded bg-destructive/20 border border-destructive/30">
                    ...
                    <p className="text-sm text-foreground/90">
                      ...
                      {" "}fast-moving earrings (especially {fastMovingTopDesign} styles) generating regular sales.
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {/* By Earring Type */}
        <>
          <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setDialogOpen("earring-type", true)}>
            <CardHeader className="pb-3">
              <CardTitle className="text-lg">Distribution by Earring Type</CardTitle>
            </CardHeader>
          </Card>
          <Dialog open={openDialogs["earring-type"]} onOpenChange={(open) => setDialogOpen("earring-type", open)}>
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Distribution by Earring Type</DialogTitle>
              </DialogHeader>
