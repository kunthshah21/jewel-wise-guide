...
import { generatePendantReport } from "@/lib/reportGenerator";
import {
  pendantInventory,
  getPendantsByLifecycleStage,
  getTotalStockValue,
  ...
  type PendantItem,
} from "@/data/pendantData";

export default function PendantDeepDive() {
  ...
  const uniqueMetals = useMemo(() => {
    ...
    pendantInventory.forEach((item) => metals.add(item.metal));
    return Array.from(metals).sort();
  }, []);

  const uniqueLocations = useMemo(() => {
    const locations = new Set<string>();
    pendantInventory.forEach((item) => locations.add(item.stockLocation));
  ...
  const deadStockPercentage = useMemo(() => 
    ((deadStockItems.length / pendantInventory.length) * 100).toFixed(1),
    [deadStockItems]
  );
  const fastMovingPercentage = useMemo(() =>
    ((fastMovingItems.length / pendantInventory.length) * 100).toFixed(1),
    [fastMovingItems]
  );
  ...
  const typePerformance = useMemo(() => 
    distributionByType.map((type) => {
      const items = pendantInventory.filter((item) => item.type === type.type);
  ...
  const designStylePerformance = useMemo(() => 
    distributionByDesignStyle.map((style) => {
      const items = pendantInventory.filter((item) => item.designStyle === style.style);
      const avgVelocity = items.length > 0 
        ? items.reduce((sum, item) => sum + item.salesVelocity, 0) / items.length
        : 0;
  ...
  if (!pendantInventory || pendantInventory.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h2 className="text-2xl font-bold">No Data Available</h2>
          <p className="text-muted-foreground">Pendant inventory data could not be loaded.</p>
  ...
  const renderInventoryTable = (items: PendantItem[]) => (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Pendant Inventory Deep Dive</h1>
            <p className="text-muted-foreground mt-1">
              Comprehensive analysis of pendant stock movement and performance
            </p>
          </div>
        </div>
        <Button
          onClick={async () => {
            try {
              await generatePendantReport({
                totalStockValue,
                totalItems: pendantInventory.length,
                deadStockItems,
                deadStockValue,
                deadStockPercentage,
                fastMovingItems,
      ...
      <div className="relative overflow-hidden rounded-2xl bg-card p-8 md:p-10 shadow-lg border-2 border-primary/30">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-4">Pendant Inventory Health Report</h2>

          <div className="space-y-2">
            <div className="bg-muted/50 rounded-lg p-4 border-l-4 border-primary">
              <p className="text-lg font-semibold text-foreground mb-2">📊 Overall Performance</p>
              <p className="text-foreground/90 leading-relaxed">
                Your pendant inventory consists of {pendantInventory.length} items valued at{" "}
            ...
            <div className="bg-muted/50 rounded-lg p-4 border-l-4 border-primary">
              ...
              <p className="text-foreground/90 leading-relaxed">
                {topPerforming.type} pendants are your star performers with {topPerforming.count} items generating
                strong sales. {worstPerforming.type} pendants need strategic intervention with only{" "}
            ...
            <div className="bg-muted/50 rounded-lg p-4 border-l-4 border-primary">
              ...
              <ul className="text-foreground/90 leading-relaxed space-y-1 list-disc list-inside">
                <li>Restock {topPerforming.type} pendants with {topPerformingDesign.style} designs - high demand detected</li>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <KPICard
          title="Total Pendant Stock Value"
          value={formatIndianCurrency(totalStockValue)}
          change={`${pendantInventory.length} items in inventory`}
          changeType="neutral"
          icon={Package}
        ...
        <KPICard
          ...
          change="Across all pendant items"
          changeType="neutral"
          icon={TrendingDown}
        ...
        <CardContent>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="fast-moving" className="mt-6">
              ...
              <div className="mb-6 rounded-lg bg-gradient-to-br from-success/10 to-success/5 border-2 border-success/30 p-5">
                ...
                <div className="bg-card/50 rounded-lg p-4 border border-success/20">
                  <p className="text-foreground/90 leading-relaxed mb-3">
                    Your {fastMovingStock.length} fast-moving pendants (valued at {formatIndianCurrency(fastMovingValue)}) are selling at 
                    {avgSalesVelocity} units/month, indicating strong market demand. {fastMovingTopType} pendants are your top performers. 
              ...
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-foreground mb-2">Fast Moving Pendants</h3>
            ...
            <TabsContent value="slow-moving" className="mt-6">
              ...
              <div className="mb-6 rounded-lg bg-gradient-to-br from-warning/10 to-warning/5 border-2 border-warning/30 p-5">
                ...
                <div className="bg-card/50 rounded-lg p-4 border border-warning/20">
                  <p className="text-foreground/90 leading-relaxed mb-3">
                    Your {slowMovingStock.length} slow-moving pendants (valued at {formatIndianCurrency(slowMovingValue)}) have been in 
              ...
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-foreground mb-2">Slow Moving Pendants</h3>
                <p className="text-sm text-muted-foreground">
                ...
                <div className="bg-card/50 rounded-lg p-4 border border-destructive/20">
                  ...
                  <div className="mt-3 p-3 rounded bg-destructive/20 border border-destructive/30">
                    ...
                    <p className="text-sm text-foreground/90">
                      ...
                      {" "}fast-moving pendants (especially {fastMovingTopDesign} styles) generating regular sales.
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {/* By Pendant Type */}
        <>
          <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setDialogOpen("pendant-type", true)}>
            <CardHeader className="pb-3">
              <CardTitle className="text-lg">Distribution by Pendant Type</CardTitle>
            </CardHeader>
          </Card>
          <Dialog open={openDialogs["pendant-type"]} onOpenChange={(open) => setDialogOpen("pendant-type", open)}>
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Distribution by Pendant Type</DialogTitle>
              </DialogHeader>
